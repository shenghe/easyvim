...
call s:defineOption('g:acp_filterTypeList', ["txt","log"])
"-----------------------------------------------------------------------------
call extend(g:acp_behavior, s:makeDefaultBehavior(), 'keep')
"-----------------------------------------------------------------------------
command! -bar -narg=0 AcpToggle  call acp#Toggle()
...
"-----------------------------------------------------------------------------
an <silent> 30.90.100 &View.Customized.Toggle\ AutoComplete :call acp#Toggle()<CR>
"-----------------------------------------------------------------------------
let Tfiletype = &ft == "" ? expand("%:e") : &ft
if g:acp_enableAtStartup && index(g:acp_filterTypeList,Tfiletype) == -1
  AcpEnable
endif
"-----------------------------------------------------------------------------
