...
function! s:File.Delete()
    ...
    if returnValue == 0
        ...
        exe 'echohl WarningMsg | echo "刪除成功!"  | echohl None'
    else
        exe 'echohl WarningMsg | echo "刪除失敗!"  | echohl None'
    endif
endfunction
...
function! s:File.Rename()
    ...
    let newName = inputdialog("請輸入新的名稱:")
    if newName == ""
        return
    endif
    ...
    if rename(lineInfo[3],Path) == 0
        ...
        exe 'echohl WarningMsg | echo "重命名失敗!"  | echohl None'
    endif
endfunction
...
function! s:Fold.Delete()
    ...
    if v:shell_error != 0
        ...
        exe 'echohl WarningMsg | echo "成功刪除!"  | echohl None'
    endif
endfunction
...
function! s:Fold.SearchByContents(...)
    let pattenStr = inputdialog("請輸入要搜索的內容:")
    if pattenStr == ""
        return
    endif
    let lineNr = exists("a:1") && type(a:1) == 0 ? a:1 : line(".")
    ...
    try
        ...
        exe 'echohl WarningMsg | echo "未找到相關內容!"  | echohl None'
    endtry
endfunction
...
function! s:Fold.SearchByNames(...)
    ...
    let lineInfo = s:GetLineInfo(lineNr)
    if !isdirectory(lineInfo[3])
         exe 'echohl WarningMsg | echo "未找到相關內容!"  | echohl None' | return
    endif
    let filesPathList = split(globpath(lineInfo[3]."/","*"),'\n')
    let resultsDict = []
...
function! s:Bookmark.Add(...)
    ...
    if writefile(BookmarkList,BookmarkPath."/.bookmark") != -1
        ...
        echohl WarningMsg | echo "添加失敗!"  | echohl None
    endif
endfunction
...
function! s:Bookmark.Delete(...)
    ...
    if founed == 1 && writefile(BookmarkList,BookmarkPath."/.bookmark") != -1
        ...
        echohl WarningMsg | echo "刪除失敗!"  | echohl None
    endif
endfunction
...
function! s:Tree.Filter()
    ...
    let filterString = inputdialog("請輸入過濾規則:",defaultString)
