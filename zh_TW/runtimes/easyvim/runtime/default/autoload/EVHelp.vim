...
function! EVHelp#setEngine() range
    let engine = inputdialog("請輸入搜索引擎名稱:",'','')
    if engine != ''
        let counts = 0
        while !exists('engineValue') || engineValue == ''
            let counts += 1
            let engineValue = inputdialog("請輸入搜索引擎鏈接({q}代表要被替換的內容)",'','')
        ...
        try
            ...
            echohl WarningMsg| echomsg "寫入失敗,請檢查文件或文件夾權限!"|echohl None
        endtry
    endif
endfunction
...
function! s:HelpGrep(word)
    ...
    try
        ...
        if exists('g:autoBrowserSearch')
            ...
            echohl WarningMsg| echomsg "沒有找到相關內容!"|echohl None
        endif
    endtry
endfunction
...
function! s:GetWord(keyword,type)
    ...
    if a:keyword == ""
        ...
        if searchContent == ''
            let searchContent = inputdialog("請輸入要搜索的內容:",'','')
        endif
    endif
...
function! EVHelp#Vimgrep()
    let word = inputdialog("請輸入您想查找的內容:")
...
function! EVHelp#Replace()
    if has("gui_running")
        ...
        let findWord = inputdialog("請輸入您想查找的內容:")
        if findWord == "" | return | endif
        let replaceWord = inputdialog("請輸入您想替換的內容:")
