...
function! EVHelp#setEngine() range
    ...
    if engine != ''
        ...
        while !exists('engineValue') || engineValue == ''
            let counts += 1
            let engineValue = inputdialog("Please input engine's link({q})",'','')
        ...
        try
            ...
            echohl WarningMsg| echomsg "Succeed!"|echohl None
        catch /.*/
...
function! EVHelp#SearchInBrowse(word,engine) range
    ...
    if engine == ""
        let engine = inputdialog("Please input the engine name:\nGoogle wikipedia msdn",'','')
    endif
...
function! s:HelpGrep(word)
    ...
    try
        ...
        if exists('g:autoBrowserSearch')
            ...
            echohl WarningMsg| echomsg "0 founed!"|echohl None
        endif
    endtry
endfunction
...
function! s:GetWord(keyword,type)
    ...
    if a:keyword == ""
        ...
        if searchContent == ''
            let searchContent = inputdialog("Please input the name:",'','')
        endif
    endif
...
function! EVHelp#Vimgrep()
    let word = inputdialog("Pease input what you will search:")
...
function! EVHelp#Replace()
    if has("gui_running")
        ...
        let findWord = inputdialog("Pease input what you will search:")
        if findWord == "" | return | endif
        let replaceWord = inputdialog("Pease input what you will replace:")
