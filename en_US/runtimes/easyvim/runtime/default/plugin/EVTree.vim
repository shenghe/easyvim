...
function! s:File.Delete()
    ...
    let allowed = confirm("Are you sure to delete it?","&OK\n&No")
    if allowed != 1
        return
    endif
    ...
    if returnValue == 0
        ...
        exe 'echohl WarningMsg | echo "Succeed!"  | echohl None'
    else
        exe 'echohl WarningMsg | echo "Failed!"  | echohl None'
    endif
endfunction
...
function! s:File.Rename()
    ...
    let newName = inputdialog("Please input the name:")
    if newName == ""
        return
    endif
...
function! s:Fold.SearchByContents(...)
    let pattenStr = inputdialog("Please input the name:")
    if pattenStr == ""
        return
    endif
    let lineNr = exists("a:1") && type(a:1) == 0 ? a:1 : line(".")
    ...
    try
        ...
        exe 'echohl WarningMsg | echo "0 founed!"  | echohl None'
    endtry
endfunction
...
function! s:Fold.SearchByNames(...)
    ...
    let lineInfo = s:GetLineInfo(lineNr)
    if !isdirectory(lineInfo[3])
         exe 'echohl WarningMsg | echo "0 founed!"  | echohl None' | return
    endif
    let filesPathList = split(globpath(lineInfo[3]."/","*"),'\n')
    let resultsDict = []
...
function! s:Bookmark.Add(...)
    ...
    for bookmark in BookmarkList
        ...
        if partList[1] == bookmarkName
            if confirm("The bookmark has existed,Are you sure to replace it?","&Yes\n&No") == 2
                return
            endif
    ...
    if writefile(BookmarkList,BookmarkPath."/.bookmark") != -1
        ...
        echohl WarningMsg | echo "Failed!"  | echohl None
    endif
endfunction
...
function! s:Tree.Filter()
    ...
    let filterString = inputdialog("Please input the filter:",defaultString)
...
"Map    {{{
inoremap <silent> <F4> <C-o>:call EVTreeToggle()<CR>
nnoremap <silent> <F4> :call EVTreeToggle()<CR>
inoremap <silent> <S-F4> <C-o>:call EVTreeToggleBookmark()<CR>
nnoremap <silent> <S-F4> :call EVTreeToggleBookmark()<CR>
"}}}
